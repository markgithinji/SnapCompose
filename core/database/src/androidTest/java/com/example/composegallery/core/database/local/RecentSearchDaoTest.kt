...
class RecentSearchDaoTest {
    ...
    fun returns_most_recent_searches_first() = runTest {
        val search1 = RecentSearch(query = "Water", timestamp = System.currentTimeMillis()).toEntity()
        val search2 =
            RecentSearch(query = "Winter Garden", timestamp = System.currentTimeMillis() + 1).toEntity()
    ...
    fun removes_specific_search_entry() = runTest {
        val search = RecentSearch(query = "Winter", timestamp = System.currentTimeMillis()).toEntity()
    ...
    fun deleteSearchIgnoreCase_deletes_case_insensitively() = runTest {
        val search = RecentSearch("Winter Garden", timestamp = 1000L).toEntity()
        dao.insertSearch(search)
    ...
    fun deleteSearch_doesNotDelete_whenCaseDoesNotMatch() = runTest {
        val search = RecentSearch("Nature", timestamp = 1000L).toEntity()
        dao.insertSearch(search)
    ...
    fun replaces_existing_query_when_inserted_again() = runTest {
        val search = RecentSearch("Duplicate", timestamp = 1000L).toEntity()
        dao.insertSearch(search)
    ...
    fun getRecentSearches_respects_limit() = runTest {
        val now = 1000L
        repeat(5) {
            dao.insertSearch(RecentSearch("Search $it", now + it).toEntity())
        }
    ...
    fun clears_all_search_entries() = runTest {
        dao.insertSearch(RecentSearch("Water", System.currentTimeMillis()).toEntity())
        dao.insertSearch(RecentSearch("Winter Garden", System.currentTimeMillis()).toEntity())
